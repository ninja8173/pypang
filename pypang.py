...
screen_width = 640
screen_height = 480
screen_size = (screen_width, screen_height) # 큐플형으로 화면 크기 지정
screen = pygame.display.set_mode(screen_size)
pygame.display.set_caption("PYPANG!")

# 배경 이미지
cu_path = os.path.dirname(__file__) #현제 팡리 폴더 경로
img_path = os.path.join(cu_path, "imges")
bg = pygame.image.load(os.path.join(img_path, "background.png"))

# 바닥 이미지
stage = pygame.image.load(os.path.join(img_path, "stage.png"))
stage_size = stage.get_rect().size #스테이지 사각형 정보
